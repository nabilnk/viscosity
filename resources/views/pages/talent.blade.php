            ...
            <div class="swiper-wrapper items-center">
                ...
                <div class="swiper-slide">
                    <div class="relative rounded-2xl overflow-hidden group cursor-pointer transition-transform duration-300">
                        <x-skeleton class="w-full h-96"><img src="{{ asset('storage/' . $talent->photo) }}" alt="{{ $talent->name }}" class="w-full h-96 object-cover opacity-0 transition-opacity duration-500" onload="this.style.opacity=1"/></x-skeleton>
                        <div class="absolute inset-0 bg-black/30 group-hover:bg-black/10 transition-all"></div>
                        <div class="absolute bottom-4 left-4 right-4 bg-white/80 backdrop-blur-sm text-black text-center font-bold p-3 rounded-xl group-hover:scale-105 transition-transform">{{ $talent->name }}</div>
                    </div>
                </div>
                @endforeach
            </div>
                ...
                <div class="swiper-slide">
                    <div class="flex flex-col md:flex-row gap-8 items-center justify-center max-w-5xl mx-auto px-4">
                        <div class="md:w-1/4 w-40 flex-shrink-0">
                            <x-skeleton class="w-full"><img src="{{ asset('storage/' . $talent->documentation_photo) }}" alt="Doc {{ $talent->name }}" class="rounded-2xl w-full shadow-lg opacity-0 transition-opacity duration-500" onload="this.style.opacity=1"/></x-skeleton>
                        </div>
                        <div class="md:w-3/4 w-full max-w-xl bg-black/40 backdrop-blur-md border border-gray-700/50 rounded-2xl p-6">
                            <h3 class="text-3xl font-bold mb-4">{{ $talent->name }}</h3>
