<x-app-layout>
    ...
    <section class="relative pt-24 pb-20 px-4 text-center bg-black/60 backdrop-blur-sm">
        ...
        <x-skeleton class="mx-auto w-2/3 sm:w-1/2 md:w-2/5">
            <img src="{{ asset('assets/logo/viscosity-text.png') }}" alt="Viscosity Logo" class="mx-auto w-2/3 sm:w-1/2 md:w-2/5 opacity-0 transition-opacity duration-500" onload="this.style.opacity=1">
        </x-skeleton>

        {{-- Hero Flyer (STATIS, Anda bisa ubah assetnya langsung) --}}
        <x-skeleton class="mx-auto my-6 w-96 h-auto rounded-xl shadow-lg">
            <img src="{{ asset('assets/home/hero-flyer.png') }}" alt="Viscosity Flyer" class="mx-auto my-6 w-96 h-auto rounded-xl shadow-lg opacity-0 transition-opacity duration-500" onload="this.style.opacity=1">
        </x-skeleton>
                    ...
                    <div class="swiper-slide" style="width: 70%">
                        <div class="bg-black/40 backdrop-blur-sm border border-gray-700/50 rounded-lg overflow-hidden">
                            <x-skeleton class="w-full h-96">
                                <img src="{{ asset('storage/assets_home/' . $record->image) }}" alt="Track Record" class="w-full h-96 object-cover opacity-0 transition-opacity duration-500" onload="this.style.opacity=1">
                            </x-skeleton>
                        </div>
                    </div>
                    @endforeach
                ...
                <div class="swiper-wrapper">
                    ...
                    <div class="swiper-slide">
                        <x-skeleton class="w-full h-auto">
                            <img src="{{ asset('storage/assets_home/' . $doc->image) }}" alt="Documentation" class="w-full h-auto object-cover opacity-0 transition-opacity duration-500" onload="this.style.opacity=1">
                        </x-skeleton>
                    </div>
                    @endforeach
                </div>
                    ...
                    <div class="swiper-slide text-center">
                        <div class="relative w-64 h-80 mx-auto bg-black/40 backdrop-blur-sm border border-gray-700/50 rounded-lg flex items-center justify-center mb-4 overflow-hidden">
                            <x-skeleton class="w-full h-full">
                                <img src="{{ asset('storage/assets_home/' . $team->image) }}" alt="{{ $team->name }}" class="w-full h-full object-cover opacity-0 transition-opacity duration-500" onload="this.style.opacity=1">
                            </x-skeleton>
                        </div>
                        <h3 class="font-semibold text-lg">{{ $team->name }}</h3>
                        <p class="text-gray-400">{{ $team->position }}</p>
                    </div>
                    @endforeach
                </div>
            </div>
            ...
            <div class="flex flex-wrap justify-center items-center gap-x-12 gap-y-8">
                ...
                <x-skeleton class="h-14">
                    <a href="{{ $partner->url }}" target="_blank">
                        <img src="{{ asset('storage/assets_home/' . $partner->image) }}" alt="{{ $partner->name }}" class="h-14 hover:scale-110 opacity-0 transition-opacity duration-500" onload="this.style.opacity=1">
                    </a>
                </x-skeleton>
                @endforeach
            </div>
    ...
    {{-- FOOTER TETAP STATIS (atau sesuaikan jika ingin dinamis juga) --}}
